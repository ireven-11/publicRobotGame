...
BombBug::BombBug(const EquipSlot equipSlot)
	:status_(std::make_shared<BombBugStatus>()),
	currentEquipSlot_(equipSlot)
{
	for (auto i = 0; i < status_->getStatusData()["bullets_pool_value"].get<int>(); i++)
	{
		bullets_.emplace_back(std::make_shared<ExplosionBullet>(status_->getAttackPower(), status_->getStatusData()["explosion_radius"].get<float>(), BulletOwnerType::PLAYER));
	}
...
void BombBug::update()
{
	...
	for (const auto& bullet : bullets_)
	{
		...
		const std::shared_ptr<BombBugStatus> bombBugStatus = std::static_pointer_cast<BombBugStatus>(status_);

		const MATRIX tempMatrix			= MV1GetFrameLocalWorldMatrix(status_->getModelHandle(), bombBugStatus->getMuzzleBoneNumber());
		const VECTOR rotationDirection	= VNorm(VGet(tempMatrix.m[1][0], tempMatrix.m[1][1], tempMatrix.m[1][2]));	//上方向を基準にしてる

		//弾を使用＋発射準備
		bulletStatus->use(MV1GetFramePosition(status_->getModelHandle(), bombBugStatus->getMuzzleBoneNumber()),
...
void BombBug::setTransform(const MATRIX& equipingMatrix)
{
	//武器の大きさを調整する
